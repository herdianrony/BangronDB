...
function switchTab(tabName, tabGroup = 'default') {
    ...
    document.querySelectorAll(`[data-tab-group="${tabGroup}"], [id^="content-"], [id^="tab-content-"]`).forEach(content => {
    ...
    document.querySelectorAll('.tab, .tab-btn, [id^="tab-"]').forEach(tab => {
        tab.classList.remove('active', 'border-blue-500', 'text-blue-400', 'border-primary-500');
    ...
    if (activeTab) {
        ...
        activeTab.classList.remove('border-transparent', 'text-gray-400', 'text-gray-500');
    }
...
function confirmDialog(message, onConfirm, onCancel) {
    ...
    modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;z-index:300;';
